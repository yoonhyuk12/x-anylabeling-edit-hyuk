...
`Images To Read Ahead`는 미리 읽을 이미지 수이며 기본값은 20, 최대 100, 0으로 설정하면 비활성화됩니다.
`Read-Ahead Cache Size (MB)`는 미리 읽은 파일을 담아 둘 메모리 상한이며 기본값은 512 MB입니다.
빠르게 넘길 때 지연이 생기면 두 값을 함께 올리세요. 이미지가 크면 캐시도 그만큼 커야 합니다.
